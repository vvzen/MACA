...
//--------------------------------------------------------------
void Firework::setup(ofPoint pos, ofColor col){
    position = pos;
    color = col;
    // give some initial y velocity to the first particle
    initial_particle.setup(pos, ofVec3f(0,9,0), true);
        ...
        if (!_exploded){
            ...
            for (int i = 0; i < 30; i++){
                ...
                new_particle.setup(
                    ...
                    ofVec3f(ofRandom(-3, 3), ofRandom(-2, 3), ofRandom(-3, 3))
                );
            ...
            for (int i = 0; i < particles.size(); i++){
                ...
                if (particles.at(i).is_active()){
                    ...
                    mesh.setVertex(i, p);
                    // set alpha
                    float alpha = ofMap(particles.at(i).current_lifetime, 0, particles.at(i).lifespan, 255, 0, true);
                    mesh.setColor(i, ofColor(color.r, color.g, color.b, alpha));
                }
