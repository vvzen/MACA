...
class Artifact{
  ...
  void setup(String _mode){
    ...
    if(mode == "plane"){
      ...
      branch(new PVector(0, 0, 0), 0, 0.0);
    }
  }
  ...
  // epitrochoid like shape : https://en.wikipedia.org/wiki/Epitrochoid
  void spiro(int numIter){
    ...
    for (int i = 0; i < numIter; i++){
      ...
      res.x = (r1 + r2) * cos(theta) - d * cos((r1 + r2) / r2 * theta);
      res.y = (r1 + r2) * sin(theta) - d * sin((r1 + r2) / r2 * theta);
      res.z += theta * 0.5;
  ...
  void branch(PVector a, int iter, float angle){

    float radius = param_A;
    ...
    if (iter < 15){
      ...
      // b.x += random(-100, 100);
      // b.y += random(-100, 100);
      // b.x += cos(a.z * a.x * 0.1) * 12-iter;
      // b.y += sin(a.z * a.y * 0.1) * 12-iter;
      
      b.x = radius * sin(angle);
      b.y = radius * cos(angle);
